...
def data_to_bitstream(num_packets_limit=None,num_databytes=None,new_number_data_bytes=False, pcap_file="F:\\DARPA1999\\Week1\\training.pcap"):
    ...
    #print the first 50 values of first packet
...
data_to_bitstream(num_packets_limit=None,num_databytes=None,new_number_data_bytes=False, pcap_file="F:\\DARPA1999\\Week1\\training.pcap")
...
def process_attacks(packet_size):
    if path.isfile("F:\\DARPA1999\\Week1\\week_4.pkl"):
        # logging.info("Using saved processed data...")
        trimmed_bits_numpy = np.load(open("F:\\DARPA1999\\Week1\\week_4.pkl", "rb"))
        return trimmed_bits_numpy
    else:
        df = pd.DataFrame(pd.read_csv(open("F:\\DARPA1999\\Week1\\week_4.csv", "r",encoding="utf-8"), sep=";", encoding="UTF-8"))
        #print(df.head())
        #print(df["id"])
        match_dict={}
        for entry in scandir("F:\\DARPA1999\\Week1\\testing"):
            attack_list=[]
            ...
            for k,v in match_dict.items():
                pcap = dpkt.pcap.Reader(open("F:\\DARPA1999\\Week1\\testing\\%s.pcap"%k,'rb'))
        ...
        # print(trimmed_bits_numpy)
        pickle.dump(trimmed_bits_numpy, open("F:\\DARPA1999\\Week1\\week_4.pkl", "wb"), protocol=4)
